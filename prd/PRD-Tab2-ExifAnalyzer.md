...
> **注意**：GPS 地图功能因依赖较重（Leaflet 38KB+）且需持续维护地图数据，已从 PRD 中移除。
...
| 功能 | 规格 | 实现 |
|------|------|------|
| 全量字段 | 相机、镜头、光圈、快门、ISO、焦距、白平衡、测光模式、闪光灯、拍摄时间、软件、GPS 坐标（文本展示） | `exifr` 全量解析 |
| 分组展示 | 按类别分组：相机信息 / 曝光参数 / 镜头信息 / GPS（文本坐标）/ 时间 / 其他 | 面板分组 UI |
...
| 大图降采样 | 4K+ 照片先缩放至 2000px 宽再统计，保证性能 | Canvas 缩放后 `getImageData` |
