...
对比 Object 与 any，Object 类型允许的变量只是允许你给它赋任意值，但是却不能够在它上面调用任意的方法，即便它真的有这些方法
...
`strictNullChecks` 默认情况下是不开启的，使用的时候可以再配置`tsconfig.json`中配置，如果再 demo 中，直接命令中加上 `--strictNullChecks` 就行。鼓励再配置中开启 strictNullChecks
